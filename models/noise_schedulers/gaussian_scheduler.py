...
class GaussianNoiseScheduler(BasicNoiseScheduler):
    ...
    def estimate_x_t_prior_from_x_0(self, x_t, t, x_0, use_log=True, clip_x0=True, var_scale=0, cold_diffusion=False):
        # x_(t-1) \sim Normal(mean, variance)
    ...
    def estimate_x_0(self, x_t, x_T, t, clip_x0=True):
        # x0 = 1 \ sqrt(a_t_bar)x_t - sqrt(1 - 1\sqrt(a_t_bar))eps()
        ndim = x_t.ndim
    ...
    def estimate_factor(self, x_0, clip_x0=True):
        x_0 = self._clip_x_0(x_0) if clip_x0 else x_0
        e = torch.randn_like(x_0)
        T = self.timesteps
        #return self.sqrt_alphas_cumprod[T - 1] * x_0 + self.sqrt_one_minus_alphas_cumprod[T - 1] * e
        return x_0
    
    #def trace_back(self, x_0, steps, clip_x0=True):
