...
class DiffusionPipeline(BasicModel):
    ...
    def training_step(self, batch: dict, batch_idx: int):
        ...
        if len(self.condition_keys) > 0:
            condition = {k:batch.get(k, None) for k in self.condition_keys}
        else:
            condition = None
        ...
        #self.log(f"train/{metric_name}", metric_val, batch_size=x_0.shape[0], on_step=True, on_epoch=True, prog_bar=True)
        self.log(f"train/loss", results['loss'], batch_size=x_0.shape[0], on_step=True, on_epoch=True, prog_bar=True)
        self.log(f"train/L1", results['L1'], batch_size=x_0.shape[0], on_step=True, on_epoch=True, prog_bar=True)
        self.log(f"train/L2", results['L2'], batch_size=x_0.shape[0], on_step=True, on_epoch=True, prog_bar=False)         
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            ...
            sample_cond = {
                'race': torch.tensor([0, 0, 1, 1], dtype=torch.int32).to(self.device)
                #'sex': torch.tensor([0, 1, 0, 1], dtype=torch.int32).to(self.device)
            }
    ...
    def validation_step(self, batch: dict, batch_idx: int):
        ...
        # ----------------- Log Scalars ----------------------
        self.log(f"val/loss", results['loss'], batch_size=x_0.shape[0], on_step=True, on_epoch=True, prog_bar=True)
        self.log(f"val/L1", results['L1'], batch_size=x_0.shape[0], on_step=True, on_epoch=True, prog_bar=True)
        self.log(f"val/L2", results['L2'], batch_size=x_0.shape[0], on_step=True, on_epoch=True, prog_bar=False)         
    ...
    @torch.no_grad()
    def sample_ctf(self, x, condition, inv = False, **kwargs):
        num_samples = x.shape[0]
        latent_emb = self.latent_embedder.encode(x) #[B, 8, 32, 32]
        if inv == True:
            factor = self.noise_scheduler.trace_back(latent_emb)
        else:
            factor = self.noise_scheduler.estimate_factor(latent_emb)
        x_0 = self.denoise(factor, condition=condition, **kwargs)
        return x_0
